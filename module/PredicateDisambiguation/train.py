...
def train(args, train_dataloader, dev_dataloader, resume=False, checkpoint=None):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.local_rank < 1 and args.save:
            ...
            save_dir = './checkpoints/%s/disambiguation/%s/' % (args.dataset_tag, mid)
...
if __name__ == "__main__":
    ...
    if args.train_path.endswith('.json'):
        train_dataloader = load_data(args.train_path, args.pretrained_model_name_or_path, args.max_tokens, True, args.local_rank)
        save_dir = args.train_path.replace(".json", '')+'/disambiguation/'+args.pretrained_model_name_or_path.split('/')[-1]
